...
data <- read.csv(file.path(dirPath, "MGMT_Infinium_BSAS_RNA_Purity.csv"))
...
data_pure <- data[which(data$FACET_Purity == 'x'),]
...
#Wilcox tests comparing across primaries or across recurrences: High purity samples only
...
#Wilcox tests comparing between primaries and recurrences: High purity samples only
wilcox.test(agdata_pure$MGMT_Infinium~agdata_pure$Tumor)
...
wilcox.test(agdata_pure$MGMT_RNA~agdata_pure$Tumor)
...
#Wilcox test for paired Infinium data: High purity samples only
...
#Wilcox test for paired RNA data: High purity samples only
